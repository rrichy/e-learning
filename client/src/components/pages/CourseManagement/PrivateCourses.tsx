...
function PrivateCourses() {
  ...
  const toggleMutation = useMutation(
    (ids: number[]) => toggleCourses("public", ids),
    {
      onSuccess: (res: any) => {
  ...
  const handleToggleStatus = async () => {
    const confirmed = await isConfirmed({
      title: "confirmation",
      content: "make all selected courses public?",
    });
  ...
  return (
    <Paper variant="outlined">
      <Stack spacing={3}>
        <Typography variant="sectiontitle2">非公開中のコース状況</Typography>
        <Button
          color="secondary"
          variant="contained"
          rounded
...
const getCourses = (filters: { [k: string]: any }) => {
  const { data, isFetching } = useQuery(
    ["private-courses-table", filters],
    async () => {
      const res = await indexCourse("private");
